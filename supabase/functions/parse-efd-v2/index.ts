...
serve(async (req) => {
  console.log("PARSE-EFD-V2 VERSION: DEBUG_V2_RECOVERY_TEST"); // Version check log
  ...
  try {
        ...
        console.log(`Created new filial: ${filialId}`);
         ...
         if (errorCode === '23505' || errorMessage.includes('duplicate key') || errorMessage.includes('unique constraint')) {
            ...
            if (duplicateFilial) {
              ...
              return new Response(
                JSON.stringify({ error: "[FATAL v2-init] Filial exists but could not be retrieved: " + errorMessage }),
                { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
              );
            }
         } else {
           ...
           return new Response(
             JSON.stringify({ error: "[FATAL v2-init] Failed to create filial: " + errorMessage }),
             { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
           );
         }
      }
    ...
    if (jobError) {
      ...
      // await supabase.storage.from("efd-files").remove([filePath]);
      return new Response(
        JSON.stringify({ error: "[FATAL v2-init] Failed to create import job: " + jobError.message + " | Code: " + jobError.code }),
        { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
